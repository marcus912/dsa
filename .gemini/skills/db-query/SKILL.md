...
```bash
uv run .gemini/skills/db-query/scripts/query_db.py query "{'difficulty': 'medium', 'status': 'todo', 'limit': 5}"
```
...
```bash
uv run .gemini/skills/db-query/scripts/query_db.py update "{'id': 1, 'status': 'solved'}"
```
